...
CATALOG = "workspace"
DATABASE = "mario_panos2"
TRAINING_TABLE = f"{CATALOG}.{DATABASE}.gold_defect_training_dataset"
...
uc_model_name = f"{catalog}.{database}.defect_lr_pipeline"
...
agg_zero_prefixes = ("count_",)
agg_null_prefixes = ("avg_", "max_", "stddev_", "vibration_1h")
...
boolean_statement = "SELECT * FROM __THIS__"
...
feature_engineering_statement = "SELECT * FROM __THIS__"
engineered_columns = []
...
def fig_confusion_matrix(y_true, y_pred, title):
    ...
    for ax, data, fmt, subtitle in [
        ...
        ax.set_xticks([0, 1])
        ax.set_yticks([0, 1])
        ax.set_xticklabels(["Correcto", "Defectuoso"])
        ax.set_yticklabels(["Correcto", "Defectuoso"])
...
def fig_lr_coefficients(coef_array, feature_names, title):
    ...
    ax.legend(
        handles = [
            Patch(color = color_positive_coef, label = "Indicativo de defecto"),
            Patch(color = color_negative_coef, label = "Indicativo de correcto")
        ],
        loc = "lower right",
        fontsize = 8
    )
...
prob_defect_column = "prob_defective"
...
def to_pandas_predictions(predictions):
    ...
    return (
        ...
        .assign(
            **{
                prob_defect_column: lambda df: df[probability_column].apply(
